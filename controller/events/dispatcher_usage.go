...
func (self *Dispatcher) registerUsageEventHandler(val interface{}, config map[string]interface{}) error {
	...
	if version == 2 {
		...
		if !ok {
			return errors.Errorf("type %v doesn't implement github.com/cosmic-cloak/ztna/controller/event/UsageEventHandler interface.", reflect.TypeOf(val))
		}
		self.AddUsageEventHandler(handler)
	} else if version == 3 {
		handler, ok := val.(event.UsageEventV3Handler)
		if !ok {
			return errors.Errorf("type %v doesn't implement github.com/cosmic-cloak/ztna/controller/event/UsageEventV3Handler interface.", reflect.TypeOf(val))
		}
